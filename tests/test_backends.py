...
class TestBackends(object):
    ...
    def run_backend_tests(
        ...
        def run_select_query(
            ...
            engine: Union[
                sqlite3.Connection, psycopg2.pool.SimpleConnectionPool
            ] = engine,
        ...
        out = list(
            db.table_select("*", "select=x", exclude_endswith=[AUDIT_END, "_metadata"])
        )
        ...
        for val in [91, "a string", {"zzz": 0}, [1, 2]]:
            out = run_update_query("set=a.k1.r2&where=z=eq.10", data=val)
            out = run_select_query("where=z=eq.10")
            assert out[0]["a"]["k1"]["r2"] == val

            out = run_update_query("set=b[0]&where=lol1=eq.456", data=val)
            out = run_select_query("where=lol1=eq.456")
            assert out[0]["b"][0] == val

            out = run_update_query("set=q.r[0|s]&where=z=eq.10", data=val)
            out = run_select_query("where=z=eq.10")
...
class TestSqlBackend(unittest.TestCase):
    ...
    def test_audit(self) -> bool:
        ...
        result = list(
            self.backend.table_select(table_name=test_table, uri_query="where=id=eq.1")
        )
        ...
        result = list(
            self.backend.table_select(table_name=audit_table(test_table), uri_query="")
        )
        ...
        result = list(
            self.backend.table_select(table_name=test_table, uri_query="where=id=eq.2")
        )
        ...
        result = list(
            self.backend.table_select(table_name=test_table, uri_query="order=id.asc")
        )
        ...
        with self.assertRaises(
            (sqlite3.OperationalError, psycopg2.errors.UndefinedTable)
        ):
            next(select)
        ...
        select = self.backend.table_select(
        ...
        audit = list(
            self.backend.table_select(table_name=audit_table(some_table), uri_query="")
        )
        ...
        result = self.backend.table_restore(
            ...
            uri_query=f"restore&primary_key=id",
        )
        self.assertEqual(len(result.get("restores")), 2)
        original = list(
            self.backend.table_select(table_name=backup_table, uri_query="")
        )
        ...
        out = list(
            self.backend.table_select(table_name=test_add_key_table, uri_query="")
        )
        ...
        out = list(
            self.backend.table_select(table_name=test_remove_key_table, uri_query="")
        )
        assert out == [data_inital]
    ...
    def test_all_view(self) -> bool:
        ...
        all_backend = self.backend_class(self.engine, schema="all", schema_pattern="p")
